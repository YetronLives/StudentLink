...
describe('ProjectDetail', () => {
  ...
  it('renders project details after fetch', async () => {
    ...
    expect(screen.getByText('A brief overview of the test project.')).toBeInTheDocument();
    expect(screen.getAllByText('Frontend').length).toBeGreaterThan(0);
    expect(screen.getByText('Node.js')).toBeInTheDocument();
    expect(screen.getByText('Learning Objectives')).toBeInTheDocument();
    expect(screen.getByText('Understand React')).toBeInTheDocument();
    expect(screen.getByText('Use Next.js')).toBeInTheDocument();
  });
